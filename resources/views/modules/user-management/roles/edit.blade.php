<x-admin-layout heading="Edit Role: {{ $role->name }}">
    <form method="POST" action="{{ route('admin.user-management.roles.update', $role->name) }}" class="max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        @csrf @method('PUT')
        <h2 class="mb-6 text-lg font-semibold text-slate-900">Informasi Role</h2>

        <div class="space-y-5">
            <x-admin.form-input name="name" label="Nama Role" :value="old('name', $role->name)" required />
                    ...
                    <div class="mb-3">
                        ...
                        <div class="ml-2 space-y-1">
                            ...
                            <label class="flex items-center gap-2 py-0.5">
                                <input type="checkbox" name="permissions[]" value="{{ $perm->name }}" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(collect(old('permissions', $role->permissions->pluck('name')->toArray()))->contains($perm->name))>
