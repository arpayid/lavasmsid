<x-admin-layout heading="Edit Pengumuman">
    <form method="POST" action="{{ route('admin.communication.announcements.update', $announcement) }}" class="max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <div class="space-y-5">
            ...
            <x-admin.form-select name="target" label="Target Penerima" :options="['all'=>'Semua','students'=>'Siswa','teachers'=>'Guru','parents'=>'Orang Tua','staff'=>'Staff']" :value="old('target',$announcement->target)" />
            <x-admin.form-select name="priority" label="Prioritas" :options="['low'=>'Rendah','normal'=>'Normal','high'=>'Tinggi','urgent'=>'Urgent']" :value="old('priority',$announcement->priority)" />
