<x-admin-layout heading="Tambah Kategori">
    <form method="POST" action="{{ route('admin.finance.categories.store') }}" class="max-w-xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        @csrf
        <div class="space-y-5">
            <x-admin.form-input name="name" label="Nama Kategori" :value="old('name')" required placeholder="contoh: SPP, Ujian, Praktik" />
