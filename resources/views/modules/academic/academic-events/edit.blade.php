<x-admin-layout heading="Tambah Event Kalender">
    <form method="POST" action="{{ route('admin.academic-events.update', $academicEvent) }}"" class="max-w-2xl rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">@csrf @method('PUT')
        <x-admin.form-input name="title" label="Judul" :value="old('title', $academicEvent->title)" required />
